window.Joe.initComment ||= (options = {}) => {
	...
	(() => {
		...
		$(document.body).on('click', '.comment-list__item .content', function () {
			...
			if (Joe.user.group == 'administrator') html = `<span class="joe_comment__operate" status="waiting" data-coid="${coid}">审核</span>丨<span class="joe_comment__operate" status="spam" data-coid="${coid}">垃圾</span>丨<span class="joe_comment__operate" status="delete" data-coid="${coid}">删除</span>丨` + html;
