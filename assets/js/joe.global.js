document.addEventListener("DOMContentLoaded", () => {
	...
	/** 文章列表缩略图加载失败自动使用主题自带缩略图 */
	{
		window.thumbOnError = function () {
			$('.thumbnail>img, .joe_detail__related-content>a>img, .graphic>.fit-cover, .joe_index__hot-list .item .link .inner .image').on('error', throttle(function () {
				// 生成一个 1 到 42 之间的随机整数
	...
	/** 头像加载失败代替 */
	{
		$('img.avatar').on('error', throttle(function () {
