		...
		<div class="joe_container">
			<div class="joe_main">
				<div class="joe_detail" data-cid="<?= $this->cid ?>">
					...
					// $db = Typecho\Db::get();
					$friends = think\facade\Db::name('friends')->where('status', 1)->whereRaw("FIND_IN_SET('single',`position`)")->order('order', 'desc')->select()->toArray();
					// $friends = $db->fetchAll($db->select()->from('table.friends')->where('status = ?', 1)->where("FIND_IN_SET('single',position)")->order('order', Typecho\Db::SORT_DESC));
					?>
