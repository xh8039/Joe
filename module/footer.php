...
?>
	<meting-js fixed="true" preload="metadata" mutex="true" volume="0.7" autotheme="true" api="<?= $this->options->JMusicApi || joe\permalink($this->options->index) . 'joe/api/meting?server=:server&type=:type&id=:id' ?>" storage="<?= $this->options->JMusicId ?>" order="<?= $this->options->JMusicOrder ?>" server="<?= $this->options->JMusicServer ?>" type="<?= $this->options->JMusicType ?>" dataId="<?= urlencode($this->options->JMusicId) ?>" <?= $this->options->JMusicPlay == 'on' ? 'autoplay="true"' : null ?>></meting-js>
<?php
}
?>
