...
?>
	<li data-coid="<?= $comments->coid ?>" class="comment-list__item" <?= $comments->status == 'waiting' ? 'style="opacity: 0.8;"' : null ?>>
		<div class="comment-list__item-contain" id="<?php $comments->theId(); ?>">
			<div class="term">
				<?php
				$mobile_handle = joe\isMobile() ? 'data-placement="right" data-trigger="focus" data-content="' . ($comments->authorId == $comments->ownerId ? '作者&nbsp;·&nbsp;' : '') . htmlentities(joe\comment_author($comments)) . '&nbsp;·&nbsp;' . joe\getAgentOS($comments->agent) . '&nbsp;·&nbsp;' . joe\getAgentBrowser($comments->agent) . '&nbsp;·&nbsp;' . joe\dateWord($comments->dateWord) . '" data-toggle="popover"' : '';
