...
<div class="joe_detail__article-video">
    ...
    <div class="featured-video-episode mt10 dplayer-featured">
        ...
        <?php foreach ($video_arr as $key => $item) :
            ...
            <a data-pic="<?= $item['pic'] ?>" data-toggle="tooltip" class="switch-video text-ellipsis" data-index="<?= $index ?>" video-url="<?= $item['url'] ?>" title="<?= $item['description'] ? $item['description'] : $video_title ?>" href="javascript:;"><span class="mr6 badg badg-sm"><?= $index ?></span><i class="episode-active-icon"></i><?= $video_title ?></a>
        <?php endforeach; ?>
    </div>
</div>