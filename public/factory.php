...
class Email
{
	public static function send($comment)
	{
		...
		$text = preg_replace_callback(
			...
			function ($match) {
				return '<img referrerpolicy="no-referrer" rel="noreferrer" style="max-height: 22px;" src="' . Helper::options()->themeUrl . '/assets/images/owo/paopao/' . str_replace('%', '', urlencode($match[1])) . '_2x.png"/>';
			},
			$text
		);
		$text = preg_replace_callback(
			...
			function ($match) {
				return '<img referrerpolicy="no-referrer" rel="noreferrer" style="max-height: 22px;" src="' . Helper::options()->themeUrl . '/assets/images/owo/aru/' . str_replace('%', '', urlencode($match[1])) . '_2x.png"/>';
			},
			$text
		);
